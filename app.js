...
function courseRowHTML(c) {
  const badges = [];
  if (schedule.includes(c.id)) badges.push('<span class="badge inschedule">In Schedule</span>');
...
function renderScheduleTray() {
  ...
  wrap.innerHTML = schedule.map(id => {
    ...
    const tags = [c._dept, isAP(c) ? 'AP' : null, c.dual_credit ? 'Dual Credit' : null].filter(Boolean).join(' \u00b7 ');
    return `
...
function computeDiplomaProgress() {
  ...
  const scheduleCourses = schedule.map(id => coursesById[id]).filter(Boolean);

  scheduleCourses.forEach(c => {
    const credits = parseCredits(c);
    let targetId = null;
    if (c.diploma_requirement_id_options) {
      targetId = c.diploma_requirement_id_options.find(id => buckets[id] && buckets[id].earned < buckets[id].credits_needed) || c.diploma_requirement_id_options[0];
    ...
    if (buckets[targetId]) buckets[targetId].earned += credits;
  });
...
function computeSeals() {
  const sc = schedule.map(id => coursesById[id]).filter(Boolean);
