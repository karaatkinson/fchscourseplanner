...
function activeProfileName() {
  const pid = getActiveProfileId();
  if (pid === 'default') return 'My Plan';
  const p = getProfiles().find(p => p.id === pid);
  return p ? p.name : 'My Plan';
}
...
function deleteProfile(id) {
  ...
  // Clean up this profile's stored data
...
function renderProfilePanel() {
  ...
  const profiles = getProfiles();
  const rows = [{ id: 'default', name: 'My Plan', isDefault: true }, ...profiles];
  list.innerHTML = rows.map(p => `
    <div class="profile-row ${p.id === activeId ? 'active' : ''}">
      ...
      ${!p.isDefault ? `<button class="profile-row-icon-btn" data-renameprofile="${p.id}" aria-label="Rename">&#9998;</button>
  ...
  return `
    <div class="schedule-row${warning ? ' has-warning' : ''}" data-id="${c.id}">
      <div class="schedule-row-main">
        ...
        ${semToggle}${fullYearToggle}${moveBtn}${moveYearSelect}
      </div>
