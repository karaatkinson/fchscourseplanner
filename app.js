...
function allYearsSchedules() {
  ...
  return ALL_YEAR_KEYS.flatMap(g => yearData[g].schedule);
}
...
function openDetail(id) {
  ...
  if (c.icc && iccRules) {
    ...
    html += `<h3>Indiana College Core</h3><div class="dc-block">${cat.label} &mdash; ${c.icc.credit_hours} credit hours${c.icc.vincennes_sourced ? ' (counts toward the 15-credit Vincennes minimum)' : ''}${c.icc.requires_ap_score ? ` &mdash; requires a score of ${c.icc.min_ap_score}+ on the AP exam` : ''}${c.icc.is_estimate ? ' <em>(estimate)</em>' : ''}</div>`;
  }
...
function renderICCView() {
  ...
  let html = `<p class="progress-intro">The Indiana College Core is a 30-credit-hour block of college coursework, guaranteed to transfer between Indiana public colleges and universities. This is an estimate based on your dual-credit and AP courses. <strong>At least 15 of the 30 credits must come directly through Vincennes University dual credit</strong> &mdash; Ivy Tech and AP credits don't count toward that 15-credit minimum, even though they can count toward the overall 30. Dual credit courses generally need a 2.0+ GPA to count. Always confirm your final plan with your counselor.</p>`;
  ...
  html += `<div class="manual-panel">
    ...
    <p>The Core generally requires at least a 2.0 GPA across your Vincennes dual-credit courses for the certificate to be awarded.</p>
    <div class="manual-field checkbox"><input type="checkbox" id="iccgpa" data-manual-key="iccGpaOk" ${manual.iccGpaOk ? 'checked' : ''}><label for="iccgpa">I have at least a 2.0 GPA across my Vincennes dual-credit courses</label></div>
  </div>`;
