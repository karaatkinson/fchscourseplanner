...
function semesterCount(sem) {
  return schedule.filter(e => {
    ...
    if (!c) return false;
    return isFullYear(c) || e.semester === sem; // full-year courses occupy a period in both semesters
  }).length;
}

function enforceSemesterCaps() {
  // Runs once after course data loads, in case a saved schedule now exceeds 7-per-semester
  ...
  if (overflow.length) {
    schedule = kept;
    alternates = overflow.concat(alternates);
    saveSchedule(); saveAlternates();
  }
}
...
function toggleCourse(id) {
  ...
  if (full) {
    if (semesterCount(1) < 7 && semesterCount(2) < 7) schedule.push({ id, semester: 1 });
    else alternates.push({ id, semester: 1 });
  } else {
    if (semesterCount(1) < 7) schedule.push({ id, semester: 1 });
    else if (semesterCount(2) < 7) schedule.push({ id, semester: 2 });
    else alternates.push({ id, semester: 1 });
  }
  saveAll();
}
function moveToSchedule(id) {
  ...
  let targetSem;
  if (full) {
    if (semesterCount(1) >= 7 || semesterCount(2) >= 7) return; // no room in one of the semesters
    targetSem = 1;
  } else {
...
function setSemester(id, sem) {
  ...
  if (entry) {
    ...
    if (semesterCount(sem) >= 7) return; // target semester already full - button should be disabled, but double-check
    entry.semester = sem;
    saveAll();
    return;
  }
  const altEntry = alternates.find(e => e.id === id);
  if (altEntry) { altEntry.semester = sem; saveAll(); }
}
...
function rowHTML(c, entry, opts) {
  ...
  const semToggle = opts.showSemToggle
    ? `<div class="sem-toggle">
         <button class="sem-btn ${entry.semester === 1 ? 'active' : ''}" data-semtoggle="${c.id}" data-sem="1" ${entry.semester !== 1 && semesterCount(1) >= 7 ? 'disabled' : ''}>S1</button>
         <button class="sem-btn ${entry.semester === 2 ? 'active' : ''}" data-semtoggle="${c.id}" data-sem="2" ${entry.semester !== 2 && semesterCount(2) >= 7 ? 'disabled' : ''}>S2</button>
       </div>`
    : '';
...
function renderScheduleTray() {
  ...
  altEl.innerHTML = alternates.length
    ? alternates.map(entry => {
        ...
        return rowHTML(c, entry, { showMove: canMove });
      }).join('')
