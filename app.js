...
function switchYear(g) {
  ...
  if (currentView === 'icc') renderICCView();
  if (currentView === 'postsecondary') renderPostsecondaryView();
}
...
async function loadAll() {
  ...
  const isSharedLink = checkForSharedLink();
  if (!isSharedLink) {
    initSetup();
    renderYearTabs();
    updateScheduleTitle();
  ...
  renderFilterBar(); // re-render now that ICC categories are available for the filter dropdown
  initViewTabs();
  initManualPanel();
  ...
  if (isSharedLink) enterReadOnlyView();
}
...
/* =========================================================
   FILTER BAR
   ========================================================= */
let filters = { duration: 'all', ap: false, dual: false, pathway: false, wbl: false, noPrereq: false, repeatable: false, diplomaReq: 'all', iccCat: 'all', sealTag: 'all' };
...
function courseMatchesFilters(c) {
  ...
  if (filters.diplomaReq !== 'all') {
    const matches = c.diploma_requirement_id === filters.diplomaReq || (c.diploma_requirement_id_options || []).includes(filters.diplomaReq);
    if (!matches) return false;
  }
  if (filters.iccCat !== 'all') {
    if (!c.icc || c.icc.category !== filters.iccCat) return false;
  }
  if (filters.sealTag !== 'all') {
    if (!(c.flexible_category_tags || []).includes(filters.sealTag)) return false;
  }
  return true;
}
...
function activeFilterCount() {
  ...
  if (filters.diplomaReq !== 'all') n++;
  if (filters.iccCat !== 'all') n++;
  if (filters.sealTag !== 'all') n++;
...
function renderFilterBar() {
  ...
  const reqOptions = Object.entries(REQUIREMENT_LABELS).filter(([k, v]) => v).map(([k, v]) => `<option value="${k}" ${filters.diplomaReq === k ? 'selected' : ''}>${v}</option>`).join('');
  const iccOptions = iccRules ? Object.entries(iccRules.categories).map(([k, v]) => `<option value="${k}" ${filters.iccCat === k ? 'selected' : ''}>${v.label}</option>`).join('') : '';
  const sealOptions = Object.entries(SEAL_TAG_LABELS).map(([k, v]) => `<option value="${k}" ${filters.sealTag === k ? 'selected' : ''}>${v}</option>`).join('');
  bar.innerHTML = `
    ...
    <div class="filter-group">
      <select class="filter-select" data-selfilter="diplomaReq"><option value="all">Diploma Requirement&hellip;</option>${reqOptions}</select>
      <select class="filter-select" data-selfilter="sealTag"><option value="all">Readiness Seal&hellip;</option>${sealOptions}</select>
      <select class="filter-select" data-selfilter="iccCat"><option value="all">Indiana College Core&hellip;</option>${iccOptions}</select>
    </div>
  ...
  bar.querySelectorAll('[data-selfilter]').forEach(sel => {
    sel.addEventListener('change', () => {
      filters[sel.dataset.selfilter] = sel.value;
      renderFilterBar(); refreshCatalogView();
    });
  });
  document.getElementById('filterClearBtn').addEventListener('click', () => {
    filters = { duration: 'all', ap: false, dual: false, pathway: false, wbl: false, noPrereq: false, repeatable: false, diplomaReq: 'all', iccCat: 'all', sealTag: 'all' };
    renderFilterBar(); refreshCatalogView();
  });
}
...
function courseRowHTML(c) {
  ...
  if (hiddenCount > 0) visibleBadges.push(`<span class="badge more" data-morebadges="${c.id}">+${hiddenCount}</span>`);
...
function initBadgeExpand() {
  document.addEventListener('click', (e) => {
    const moreBtn = e.target.closest('.badge.more');
    if (!moreBtn) return;
    e.stopPropagation();
    const c = coursesById[moreBtn.dataset.morebadges];
    if (!c) return;
    const container = moreBtn.closest('.badges');
    container.innerHTML = buildBadges(c).join('');
  }, true); // capture phase - must run before the course-row's own bubble-phase click listener opens the detail panel
}
...
function initViewTabs() {
  document.querySelectorAll('.view-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      document.getElementById('postsecondaryView').style.display = view === 'postsecondary' ? '' : 'none';
      if (view === 'progress') renderProgressView();
      if (view === 'icc') renderICCView();
      if (view === 'postsecondary') renderPostsecondaryView();
      updateScheduleCollapseUI();
    });
  });
}
...
const MANUAL_DEFAULTS = {
  ...
  ps: {
    studentName: '', gradYear: '', studentEmail: '', personalEmail: '',
    byYear: { JH: {}, 9: {}, 10: {}, 11: {}, 12: {} }
  }
};
function loadManual() {
  try {
    const raw = localStorage.getItem(profileKey('fchs-manual-inputs'));
    const m = raw ? { ...MANUAL_DEFAULTS, ...JSON.parse(raw) } : { ...MANUAL_DEFAULTS };
    ensurePsShape(m);
    return m;
  } catch (e) { return { ...MANUAL_DEFAULTS }; }
}
function ensurePsShape(m) {
  if (!m.ps) m.ps = { studentName: '', gradYear: '', studentEmail: '', personalEmail: '', byYear: {} };
  if (!m.ps.byYear) m.ps.byYear = {};
  ALL_YEAR_KEYS.forEach(g => { if (!m.ps.byYear[g]) m.ps.byYear[g] = {}; });
}
function saveManual() {
...
function computeDiplomaProgress() {
  ...
  entries.forEach(entry => {
    ...
    if (c.diploma_requirement_id_options) {
      ...
      // Some courses have a fallback bucket if their primary one is already full - e.g. per the state eligibility
      // memo, any Biology/Computer Science/Additional Science course can also satisfy STEM-Focused credit once
      // its own primary science requirement is already met by another course. Only used once primary is FULL,
      // never in preference to it (unlike diploma_requirement_id_options, which picks whichever is scarcer).
      if (buckets[targetId].earned >= buckets[targetId].credits_needed && c.diploma_requirement_id_fallback && buckets[c.diploma_requirement_id_fallback]) {
        targetId = c.diploma_requirement_id_fallback;
      }
    } else {
...
function computeICCProgress() {
  const entries = allYearsSchedules().filter(e => coursesById[e.id] && coursesById[e.id].icc);
  const categoryTotals = {};
  Object.entries(iccRules.categories).forEach(([k, v]) => { categoryTotals[k] = { earned: 0, label: v.label, min: v.min_credits, max: v.max_credits, contributions: [] }; });

  let totalCredits = 0, vincennesCredits = 0;
  const apCoursesNeedingScore = [];
  const countedCourses = [];
  const totalContributions = [];
  const vincennesContributions = [];
  ...
  entries.forEach(entry => {
    ...
    if (applied > 0) {
      cat.contributions.push({ title: c.title, credits: applied, year: entry._year, vincennes: icc.vincennes_sourced, isEstimate: icc.is_estimate });
      totalContributions.push({ title: c.title, credits: applied, year: entry._year, note: cat.label });
      if (icc.vincennes_sourced) vincennesContributions.push({ title: c.title, credits: applied, year: entry._year, note: cat.label });
    }
    countedCourses.push({ id: c.id, title: c.title, credits: applied, vincennes: icc.vincennes_sourced, isEstimate: icc.is_estimate });
  });

  return { categoryTotals, totalCredits, vincennesCredits, apCoursesNeedingScore, countedCourses, totalContributions, vincennesContributions };
}
...
function renderICCView() {
  ...
  html += `<h2 class="progress-section-title">Overall Progress <span class="coming-soon">(click a card to see which courses count)</span></h2><div class="diploma-grid">
    <div class="prog-card" data-icctotal="total" style="cursor:pointer;">
    ...
    <div class="prog-card" data-icctotal="vincennes" style="cursor:pointer;">
  ...
  html += `<h2 class="progress-section-title">By Category <span class="coming-soon">(click a card to see which courses count)</span></h2><div class="diploma-grid">`;
  Object.entries(r.categoryTotals).forEach(([catKey, cat]) => {
    const pct = Math.min(100, Math.round((cat.earned / cat.max) * 100));
    const meetsMin = cat.earned >= cat.min;
    html += `<div class="prog-card" data-icccat="${catKey}" style="cursor:pointer;">
  ...
  el.innerHTML = html;
  Object.entries(r.categoryTotals).forEach(([catKey, cat]) => {
    const cardEl = el.querySelector(`[data-icccat="${catKey}"]`);
    if (cardEl) cardEl.addEventListener('click', () => openRequirementTray(cat.label, { earned: cat.earned, needed: cat.min, contributions: cat.contributions }));
  });
  const totalCard = el.querySelector('[data-icctotal="total"]');
  if (totalCard) totalCard.addEventListener('click', () => openRequirementTray('Total ICC Credits', { earned: r.totalCredits, needed: 30, contributions: r.totalContributions }));
  const vinCard = el.querySelector('[data-icctotal="vincennes"]');
  if (vinCard) vinCard.addEventListener('click', () => openRequirementTray('Vincennes-Sourced Credits', { earned: r.vincennesCredits, needed: 15, contributions: r.vincennesContributions }));
}

/* =========================================================
   POSTSECONDARY PLAN
   ========================================================= */
const XELLO_URL = 'https://login.xello.world/'; // general Xello login - swap in FCHS's specific SSO/portal link if one exists

const PS_FIELDS = {
  JH: [
    { key: 'intent', label: 'When I graduate, I plan to:', type: 'select' },
    { key: 'careerClusters', label: 'Top 3 Career Clusters from Xello', type: 'textarea' },
    { key: 'careersInterested', label: '3 Careers that Interest Me', type: 'textarea' },
    { key: 'whyInterested', label: 'Why am I interested in these careers?', type: 'textarea' }
  ],
  9: [
    { key: 'intent', label: 'When I graduate, I plan to:', type: 'select' },
    { key: 'careersInterested', label: '3 Careers that Interest Me', type: 'textarea' },
    { key: 'trainingNeeded', label: 'What College or Postsecondary Training is needed for these careers? (Use Xello to research)', type: 'textarea' }
  ],
  10: [
    { key: 'intent', label: 'When I graduate, I plan to:', type: 'select' },
    { key: 'careersSkills', label: '3 Careers that Interest Me: What skills do I need to enter these careers?', type: 'textarea' },
    { key: 'collegesConsidering', label: 'Colleges or Postsecondary Training I am considering', type: 'textarea' },
    { key: 'employabilitySkills', label: 'How are you developing employability skills? (Extracurriculars, Work Experience, Volunteering, etc.)', type: 'textarea' }
  ],
  11: [
    { key: 'intent', label: 'When I graduate, I plan to:', type: 'select' },
    { key: 'careersSkillsGap', label: 'Top 2 Careers that Interest Me: Do I have the skills needed for these careers? If not, what am I doing to build them?', type: 'textarea' },
    { key: 'collegesConsidering', label: 'Colleges or Postsecondary Training I am considering', type: 'textarea' },
    { key: 'certifications', label: 'Certifications I am earning', type: 'textarea' },
    { key: 'employabilitySkills', label: 'How are you developing employability skills? (Extracurriculars, Work Experience, Volunteering, etc.)', type: 'textarea' }
  ],
  12: [
    { key: 'intent', label: 'When I graduate, I plan to:', type: 'select' },
    { key: 'careersSkillsGap', label: '2 Careers that Interest Me: Do I have the skills needed for these careers? If not, what am I doing to build them?', type: 'textarea' },
    { key: 'collegesApplyingTo', label: 'Colleges and Postsecondary Training I am applying to', type: 'textarea' },
    { key: 'certifications', label: 'Certifications I am earning', type: 'textarea' }
  ]
};
const PS_INTENT_OPTIONS = ['', 'Enroll (college)', 'Enlist (military)', 'Employ (job)'];

function psOnTrackHTML() {
  const catTotals = computeDiplomaProgress();
  const diplomaMet = Object.values(catTotals).every(c => c.earned >= c.needed);
  const seals = computeSeals();
  const rows = [
    { label: 'Indiana Diploma', met: diplomaMet, tracked: true },
    { label: 'Enrollment Seal (Honors tier)', met: seals[0].items.every(i => i.met), tracked: true },
    { label: 'Enrollment Plus Seal', tracked: false },
    { label: 'Enlistment Seal (Honors tier)', met: seals[2].items.every(i => i.met), tracked: true },
    { label: 'Enlistment Plus Seal', tracked: false },
    { label: 'Employment Seal (Honors tier)', met: seals[1].items.every(i => i.met), tracked: true },
    { label: 'Employment Plus Seal', tracked: false }
  ];
  return `<div class="ps-ontrack">
    ${rows.map(r => `<div class="ps-ontrack-row">
      <span class="ps-ontrack-check ${r.tracked ? (r.met ? 'met' : 'unmet') : 'na'}">${r.tracked ? (r.met ? '\u2713' : '\u2013') : '?'}</span>
      <span>${r.label}${!r.tracked ? ' <em>(not yet tracked in this tool)</em>' : ''}</span>
    </div>`).join('')}
  </div>`;
}

function renderPostsecondaryView() {
  const el = document.getElementById('postsecondaryView');
  const ps = manual.ps;
  const fields = PS_FIELDS[activeGrade] || [];
  const showOnTrack = activeGrade !== JH_KEY;

  let html = `<p class="progress-intro">This mirrors FCHS's Postsecondary Plan. Fill it out here, then use Print / Save to generate a copy to share with your counselor.</p>`;

  html += `<div class="ps-topfields">
    <div class="ps-field"><label>Student Name</label><input type="text" data-psmeta="studentName" value="${ps.studentName || ''}"></div>
    <div class="ps-field"><label>Year of Graduation</label><input type="text" data-psmeta="gradYear" value="${ps.gradYear || ''}"></div>
    <div class="ps-field"><label>Student Email</label><input type="text" data-psmeta="studentEmail" value="${ps.studentEmail || ''}"></div>
    <div class="ps-field"><label>Personal Email</label><input type="text" data-psmeta="personalEmail" value="${ps.personalEmail || ''}"></div>
  </div>`;

  html += `<h2 class="progress-section-title">${GRADE_LABELS[activeGrade]}</h2>`;
  html += `<p class="ps-xello-link"><a href="${XELLO_URL}" target="_blank" rel="noopener">Open Xello \u2197</a> &nbsp;to research careers, clusters, and build your resume before filling this out.</p>`;

  const yearData_ps = manual.ps.byYear[activeGrade] || {};
  html += `<div class="ps-fields">` + fields.map(f => {
    const val = yearData_ps[f.key] || '';
    if (f.type === 'select') {
      return `<div class="ps-field"><label>${f.label}</label>
        <select data-psfield="${f.key}">${PS_INTENT_OPTIONS.map(o => `<option value="${o}" ${val === o ? 'selected' : ''}>${o || 'Select one\u2026'}</option>`).join('')}</select>
      </div>`;
    }
    return `<div class="ps-field"><label>${f.label}</label><textarea data-psfield="${f.key}" rows="3">${val}</textarea></div>`;
  }).join('') + `</div>`;

  if (showOnTrack) {
    html += `<h3 class="ps-subhead">I am on track for: <span class="coming-soon">(computed automatically from your schedule)</span></h3>`;
    html += psOnTrackHTML();
  }

  if (activeGrade === 12) {
    html += `<h3 class="ps-subhead">Ordering Transcripts &amp; Scores</h3>
      <ul class="ps-links">
        <li>FCHS Transcript: <a href="https://www.parchment.com" target="_blank" rel="noopener">parchment.com</a></li>
        <li>Ivy Tech Transcript: <a href="https://www.ivytech.edu/student-services/registrar-transcripts/transcripts/" target="_blank" rel="noopener">ivytech.edu</a></li>
        <li>Vincennes University Transcript: <a href="https://www.vinu.edu/registrar-s-office/transcripts.html" target="_blank" rel="noopener">vinu.edu</a></li>
        <li>AP / SAT Scores: <a href="https://www.collegeboard.org" target="_blank" rel="noopener">collegeboard.org</a></li>
      </ul>`;
  }

  el.innerHTML = html;

  el.querySelectorAll('[data-psmeta]').forEach(input => {
    input.addEventListener('change', () => { manual.ps[input.dataset.psmeta] = input.value; saveManual(); });
  });
  el.querySelectorAll('[data-psfield]').forEach(input => {
    input.addEventListener('change', () => {
      manual.ps.byYear[activeGrade][input.dataset.psfield] = input.value;
      saveManual();
    });
  });
}
...
function buildPrintPostsecondaryPage(grade) {
  const ps = manual.ps;
  const fields = PS_FIELDS[grade] || [];
  const yearVals = ps.byYear[grade] || {};
  const fieldRows = fields.map(f =>
    `<tr><td style="width:38%;"><strong>${f.label}</strong></td><td>${(yearVals[f.key] || '').replace(/\n/g, '<br>') || '<em>(not answered)</em>'}</td></tr>`
  ).join('');
  ...
  if (grade !== JH_KEY) {
    ...
    const seals = computeSeals();
    const rows = [
      ['Indiana Diploma', diplomaMet, true],
      ['Enrollment Seal (Honors tier)', seals[0].items.every(i => i.met), true],
      ['Enrollment Plus Seal', false, false],
      ['Enlistment Seal (Honors tier)', seals[2].items.every(i => i.met), true],
      ['Enlistment Plus Seal', false, false],
      ['Employment Seal (Honors tier)', seals[1].items.every(i => i.met), true],
      ['Employment Plus Seal', false, false]
    ];
    onTrackBlock = `<p class="print-section-title" style="font-size:1rem;margin-top:18px;">On Track For</p><ul>` +
      rows.map(([label, met, tracked]) => `<li>${tracked ? (met ? '\u2713' : '\u2013') : '?'} ${label}${!tracked ? ' (not yet tracked in this tool)' : ''}</li>`).join('') + `</ul>`;
  }

  const transcriptBlock = grade === 12
    ? `<p class="print-note" style="margin-top:14px;">Transcripts: parchment.com (FCHS) &middot; ivytech.edu (Ivy Tech) &middot; vinu.edu (Vincennes) &middot; collegeboard.org (AP/SAT)</p>`
    : '';

  return `<div class="print-page">
    ${printHeaderHTML()}
    <p class="print-section-title">Postsecondary Plan \u2014 ${GRADE_LABELS[grade]}</p>
    <table class="print-table"><tbody>${fieldRows}</tbody></table>
    ${onTrackBlock}
    ${transcriptBlock}
  </div>`;
}
...
function generatePrintDoc() {
  ...
  const includePostsecondary = document.getElementById('printPostsecondary').checked;
  ...
  if (includePostsecondary) selectedYears.forEach(g => { pages += buildPrintPostsecondaryPage(g); });
...
/* =========================================================
   SHARE LINK (URL-encoded snapshot, no server/backend needed)
   ========================================================= */
...
function buildSharePayload() {
  yearData[activeGrade] = { schedule, alternates, status: yearData[activeGrade].status };
  return { v: 1, name: activeProfileName(), mode, currentGrade, yearData, manual };
}
function generateShareLink() {
  const encoded = encodeShareData(buildSharePayload());
  return `${window.location.origin}${window.location.pathname}?share=${encoded}`;
}
function initShare() {
  document.getElementById('shareOpenBtn').addEventListener('click', () => {
    document.getElementById('shareLinkOutput').value = generateShareLink();
  ...
  document.getElementById('shareCopyBtn').addEventListener('click', () => {
    ...
    }).catch(() => { /* clipboard API unavailable - the selected text is still there for manual copy */ });
  });
}
...
/* =========================================================
   ...
   ========================================================= */
let readOnlyPayload = null;

function checkForSharedLink() {
  const params = new URLSearchParams(window.location.search);
  const shared = params.get('share');
  if (!shared) return false;
  const payload = decodeShareData(shared);
  if (!payload || !payload.yearData) return false;
  readOnlyPayload = payload;
  return true;
}

function enterReadOnlyView() {
  // Point the normal global state at the shared data (never persisted anywhere while in this mode)
  mode = readOnlyPayload.mode;
  currentGrade = readOnlyPayload.currentGrade;
  yearData = readOnlyPayload.yearData;
  ALL_YEAR_KEYS.forEach(g => { if (!yearData[g]) yearData[g] = { schedule: [], alternates: [], status: 'planned' }; });
  activeGrade = mode === 'builder' ? currentGrade : 9;
  schedule = yearData[activeGrade].schedule;
  alternates = yearData[activeGrade].alternates;
  manual = { ...MANUAL_DEFAULTS, ...(readOnlyPayload.manual || {}) };
  ...
  document.getElementById('progressView').style.display = 'none';
  document.getElementById('iccView').style.display = 'none';
  document.querySelector('nav.view-tabs').style.display = 'none';
  document.getElementById('yearTabs').style.display = 'none';
  document.getElementById('scheduleTray').style.display = 'none';

  document.getElementById('readOnlyName').textContent = `${readOnlyPayload.name || 'a shared plan'}\u2019s plan`;
  document.getElementById('readOnlyBanner').style.display = 'flex';
  renderReadOnlyView();
}

function renderReadOnlyView() {
  const el = document.getElementById('readOnlyView');
  let html = '';

  ALL_YEAR_KEYS.forEach(g => {
    if (!yearData[g].schedule.length && !yearData[g].alternates.length) return;
    const sem1 = [], sem2 = [], alt = [];
    yearData[g].schedule.forEach(entry => {
      const c = coursesById[entry.id];
      if (!c) return;
      const notes = [isAP(c) ? 'AP' : null, c.dual_credit ? 'Dual Credit' : null].filter(Boolean).join(', ');
      const row = `<tr><td>${c.title}</td><td>${c.credits}</td><td>${notes}</td></tr>`;
      if (entryIsFullYear(entry, c)) { sem1.push(row); sem2.push(row); }
      else if (entry.semester === 2) sem2.push(row); else sem1.push(row);
    });
    yearData[g].alternates.forEach(entry => { const c = coursesById[entry.id]; if (c) alt.push(`<li>${c.title}</li>`); });
    html += `<div class="ro-section">
      <h2 class="ro-section-title">${GRADE_LABELS[g]}</h2>
      <p class="ro-meta">${yearData[g].status === 'completed' ? 'Completed' : 'Planning'}</p>
      <table class="ro-table"><thead><tr><th>Semester 1</th><th>Credits</th><th>Notes</th></tr></thead><tbody>${sem1.join('') || '<tr><td colspan="3">No courses</td></tr>'}</tbody></table>
      <table class="ro-table"><thead><tr><th>Semester 2</th><th>Credits</th><th>Notes</th></tr></thead><tbody>${sem2.join('') || '<tr><td colspan="3">No courses</td></tr>'}</tbody></table>
      ${alt.length ? `<p class="ro-meta"><strong>Alternatives:</strong></p><ul>${alt.join('')}</ul>` : ''}
    </div>`;
  });
  ...
  if (diplomaRules) {
    const catTotals = computeDiplomaProgress();
    const seals = computeSeals();
    const rows = Object.entries(catTotals).map(([k, cat]) => `<tr><td>${CATEGORY_LABELS[k]}</td><td>${cat.earned} / ${cat.needed}</td><td>${cat.earned >= cat.needed ? 'Met' : 'Not yet met'}</td></tr>`).join('');
    const sealRows = seals.map(seal => `<tr><td>${seal.name}</td><td>${seal.items.every(i => i.met) ? 'On track' : 'In progress'}</td></tr>`).join('');
    html += `<div class="ro-section">
      <h2 class="ro-section-title">Indiana Diploma Progress</h2>
      <table class="ro-table"><thead><tr><th>Requirement</th><th>Credits</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>
      <table class="ro-table"><thead><tr><th>Readiness Seal</th><th>Status</th></tr></thead><tbody>${sealRows}</tbody></table>
    </div>`;
  }

  if (iccRules) {
    const r = computeICCProgress();
    const catRows = Object.values(r.categoryTotals).map(cat => `<tr><td>${cat.label}</td><td>${cat.earned} cr.</td></tr>`).join('');
    html += `<div class="ro-section">
      <h2 class="ro-section-title">Indiana College Core Progress</h2>
      <p class="ro-meta">Total: ${r.totalCredits} / 30 &nbsp;|&nbsp; Vincennes-sourced: ${r.vincennesCredits} / 15</p>
      <table class="ro-table"><thead><tr><th>Category</th><th>Earned</th></tr></thead><tbody>${catRows}</tbody></table>
    </div>`;
  }

  el.innerHTML = html || `<p class="ro-meta">This shared plan doesn't have any courses in it yet.</p>`;
  el.style.display = 'block';
}

function initSaveCopy() {
  document.getElementById('saveCopyBtn').addEventListener('click', openSaveCopyModal);
  document.getElementById('saveCopyCancelBtn').addEventListener('click', () => {
    document.getElementById('saveCopyOverlay').style.display = 'none';
  });
  document.getElementById('saveCopyNewBtn').addEventListener('click', () => {
    const name = document.getElementById('saveCopyNewName').value.trim();
    if (!name) { alert('Enter a name for this plan.'); return; }
    saveSharedAsNewProfile(name);
  });
}
function openSaveCopyModal() {
  const sharedName = readOnlyPayload.name || 'Shared Plan';
  document.getElementById('saveCopyNewName').value = sharedName;

  // Look for an existing profile with a matching name, on this device
  const existing = [{ id: 'default', name: getDefaultProfileName() }, ...getProfiles()].find(p => p.name.toLowerCase() === sharedName.toLowerCase());
  const replaceWrap = document.getElementById('saveCopyReplaceOption');
  if (existing) {
    replaceWrap.style.display = 'block';
    const btn = document.getElementById('saveCopyReplaceBtn');
    btn.textContent = `Replace "${existing.name}"`;
    btn.onclick = () => replaceSharedIntoProfile(existing.id);
  } else {
    replaceWrap.style.display = 'none';
  }
  document.getElementById('saveCopyOverlay').style.display = 'flex';
}
function writeProfileData(id, payload) {
  const write = (base, value) => {
    const key = id === 'default' ? base : `fchs-profile-${id}-${base}`;
    localStorage.setItem(key, value);
  };
  write('fchs-mode', payload.mode || 'builder');
  write('fchs-current-grade', String(payload.currentGrade || 9));
  write('fchs-yeardata', JSON.stringify(payload.yearData));
  write('fchs-manual-inputs', JSON.stringify({ ...MANUAL_DEFAULTS, ...(payload.manual || {}) }));
}
function saveSharedAsNewProfile(name) {
  const id = 'p' + Date.now() + Math.floor(Math.random() * 1000);
  writeProfileData(id, readOnlyPayload);
  const profiles = getProfiles();
  profiles.push({ id, name, createdAt: new Date().toISOString() });
  saveProfilesList(profiles);
  exitReadOnlyAndSwitchTo(id);
}
function replaceSharedIntoProfile(id) {
  if (!confirm(`This will overwrite the current data in this plan with the shared plan's data. Continue?`)) return;
  writeProfileData(id, readOnlyPayload);
  exitReadOnlyAndSwitchTo(id);
}
function exitReadOnlyAndSwitchTo(profileId) {
  document.getElementById('saveCopyOverlay').style.display = 'none';
  document.getElementById('readOnlyBanner').style.display = 'none';
  document.getElementById('readOnlyView').style.display = 'none';
  document.querySelector('nav.view-tabs').style.display = '';
  document.getElementById('yearTabs').style.display = '';
  document.getElementById('scheduleTray').style.display = '';
  // Clear the ?share= parameter so refreshing the page doesn't re-enter read-only mode
  window.history.replaceState({}, '', window.location.pathname);
  readOnlyPayload = null;
  switchProfile(profileId, true); // skipPersist - nothing real to persist, we were in a temporary read-only state
}
